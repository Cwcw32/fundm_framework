...
def train(args):
    ...
    for epoch in range(start_epoch, args.epoch_num + 1):
        ...
        batch_generator_train_2 = Data2.generate_fi_batches_2(dataset=train_dataset_2, batch_size=args.batch_size)

        #################
        ### 有padding版本
        #################
        #
        # for item1, item2 in tqdm(zip(batch_generator_train, batch_generator_train_2)):
        ...
        #     S_A_query_mask = F.pad(S_A_query_mask, pad=(0, len(S_A_query_mask_2[0]) - len(S_A_query_mask[0])),
        #                            mode='constant', value=0)
        #     S_A_query_seg = F.pad(S_A_query_seg, pad=(0, len(S_A_query_seg_2[0]) - len(S_A_query_seg[0])),
        #                           mode='constant', value=1)
        #     S_A_answer_start = F.pad(S_A_answer_start, pad=(0, len(S_A_answer_start_2[0]) - len(S_A_answer_start[0])),
        #                              mode='constant', value=-1)
        #     S_A_answer_end = F.pad(S_A_answer_end, pad=(0, len(S_A_answer_end_2[0]) - len(S_A_answer_end[0])),
        #                            mode='constant', value=-1)
        #
        ...
        #                       mode='constant', value=0)
        #     A_O_query_mask = F.pad(A_O_query_mask, pad=(0, len(A_O_query_mask_2[0][0]) - len(A_O_query_mask[0][0]), 0,
        #                                                 len(A_O_query_mask_2[0]) - len(A_O_query_mask[0])),
        #                            mode='constant', value=0)
        #     A_O_query_seg = F.pad(A_O_query_seg, pad=(0, len(A_O_query_seg_2[0][0]) - len(A_O_query_seg[0][0]), 0, 0),
        #                           mode='constant', value=1)
        #     A_O_query_seg = F.pad(A_O_query_seg, pad=(0, 0, 0, len(A_O_query_seg_2[0]) - len(A_O_query_seg[0])),
        #                           mode='constant', value=0)
        #     A_O_answer_start = F.pad(A_O_answer_start, pad=(
        #         0, len(A_O_answer_start_2[0][0]) - len(A_O_answer_start[0][0]), 0,
        #         len(A_O_answer_start_2[0]) - len(A_O_answer_start[0])), mode='constant', value=-1)
        #     A_O_answer_end = F.pad(A_O_answer_end, pad=(0, len(A_O_answer_end_2[0][0]) - len(A_O_answer_end[0][0]), 0,
        #                                                 len(A_O_answer_end_2[0]) - len(A_O_answer_end[0])),
        #                            mode='constant', value=-1)
        ...
        if True:
            ...
            if epoch % 4 == 0:
                ...
                dev_f2 = test3(model, tokenizer_1, batch_generator_dev_2, dev_standard, 0.8,
                               logger)  # ,gpu='cuda', max_len=200,task_type=task_type)
            ...
            # #######debug
            if epoch % 4 == 0:
                batch_generator_test_2 = Data2.generate_fi_batches_2(dataset=test_dataset_2, batch_size=1,
                                                                     shuffle=False)
                logger.info("test_debug")
                test_f2 = test3(model, tokenizer_1, batch_generator_test_2, test_standard, 0.8, logger)
