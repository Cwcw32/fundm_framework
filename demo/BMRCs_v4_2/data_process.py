    ...
    def __init__(self,
        ...
        if self.task_type=='ASQP':#os.path.exists('xxx/xxx/filename') is False:  # 如果没处理过，那就处理
            ...
            for qa_index, qa in enumerate(self.QAs):
                ...
                P_A_QUERY = qa['P_A_QUERY']
                P_A_ANSWER = qa['P_A_ANSWER']

                P_O_QUERY = qa['P_O_QUERY']
                P_O_ANSWER = qa['P_O_ANSWER']

                PA_O_QUERY = qa['PA_O_QUERY']
                PA_O_ANSWER = qa['PA_O_ANSWER']

                PO_A_QUERY = qa['PO_A_QUERY']
                PO_A_ANSWER = qa['PO_A_ANSWER']
                ...
                if opt.model_name=='ROBMRC':
                    ...
                    yingshe_origin=[ll for ll in range(0,len(text_lower_))]  # 去除CLS的影响
                    text_length=yingshe_origin[-1] + 1
                    bert_length = len(yingshe_origin)
                ...
                gat_aspect_indi=[]
                gat_bert_sequence=[]
                gat_bert_segments_ids=[]
                gat_dependency_tree=[]

                # rethink
                # P->A
                _forward_P_A_query = []
                _forward_P_A_query_mask = []
                _forward_P_A_query_seg = []
                _forward_P_A_answer_start = []
                _forward_P_A_answer_end = []
                # P->O
                _forward_P_O_query = []
                _forward_P_O_query_mask = []
                _forward_P_O_query_seg = []
                _forward_P_O_answer_start = []
                _forward_P_O_answer_end = []
                # PA->O
                _forward_PA_O_query = []
                _forward_PA_O_query_mask = []
                _forward_PA_O_query_seg = []
                _forward_PA_O_answer_start = []
                _forward_PA_O_answer_end = []
                # PO->A
                _forward_PO_A_query = []
                _forward_PO_A_query_mask = []
                _forward_PO_A_query_seg = []
                _forward_PO_A_answer_start = []
                _forward_PO_A_answer_end = []
                ...
                # 获得aspect
                if opt.gat is True:
                    for a_item in S_A_ANSWER:
                        print(1)
                        for AO_P_index, AO_P_item in enumerate(AO_P_QUERY):
                            a_start= a_item[0].index(1)
                            a_end=a_item[1].index(1)

                            text_raw_bert_indices = T_text
                            # 参考BERT-SPC的，[CLS]+W+[SEP]+ASPECT+[SEP]
                            # select_spans_list = []
                            asp = text[a_start:a_end+1]

                            asp_bert_ids=self.tokenizer.convert_tokens_to_ids(asp)
                            bert_sequence = T_CLS + text_raw_bert_indices + T_SEP + asp_bert_ids
                            bert_segments_ids = [0] * (bert_length + 2) + [1] * (len(asp_bert_ids) )
                            bert_mask=[1]*len(bert_sequence)

                            bert_sequence = bert_sequence
                            bert_segments_ids = bert_segments_ids

                            gat_bert_segments_ids.append(bert_segments_ids)
                            gat_bert_sequence.append(bert_sequence)

                            aspect_indi = [0] * text_length
                            for pidx in range(a_start,a_end+1):
                                aspect_indi[pidx] = 1

                            gat_aspect_indi.append(aspect_indi)

                            ##################
                            ### 待更改
                            ##################
                            dep=torch.ones(text_length)
                            gat_dependency_tree.append(dep)
                ############# GAT END
                ...
                if opt.model_name=='BMRC':
                    ...
                    T_Q_S_A = self.tokenizer.convert_tokens_to_ids([word_.lower() for word in S_A_QUERY for word_ in word])
                    if nizhuan is True:
                        _forward_S_A_query = T_CLS + T_text + T_SEP + T_Q_S_A
                        _S_A_query_mask_temp = [1] * len(_forward_S_A_query)  # 因为就1个，直接取0就行
                        _S_A_query_seg_temp = [0] * (len(T_text)+2) + [1] * (len(_forward_S_A_query) - len(T_text)-2)
                        _forward_S_A_query_mask = _S_A_query_mask_temp
                        _forward_S_A_query_seg = _S_A_query_seg_temp

                        _forward_S_A_answer_start_temp = [-1]+S_A_ANSWER[0][0]+ [-1] * (len(T_Q_S_A) + 1)
                        _forward_S_A_answer_end_temp = [-1]+S_A_ANSWER[0][1]+ [-1] * (len(T_Q_S_A) + 1)
                    ...
                    if nizhuan is True:
                        _forward_S_O_query = T_CLS + T_text + T_SEP + T_Q_S_O
                        _S_O_query_mask_temp = [1] * len(_forward_S_O_query)  # 因为就1个，直接取0就行
                        _S_O_query_seg_temp = [0] * (len(T_text)+2) + [1] * (len(_forward_S_O_query) - len(T_text)-2)
                        _forward_S_O_query_mask = _S_O_query_mask_temp
                        _forward_S_O_query_seg = _S_O_query_seg_temp

                        _forward_S_O_answer_start_temp = [-1]+S_O_ANSWER[0][0]+ [-1] * (len(T_Q_S_O) + 1)
                        _forward_S_O_answer_end_temp = [-1]+S_O_ANSWER[0][1]+ [-1] * (len(T_Q_S_O) + 1)
                        ...
                        assert len(T_Q_S_O) == len(_S_O_query_mask_temp)
                        assert len(T_Q_S_O) == len(_S_O_query_seg_temp)
                    ...
                    # 好像不需要考虑多组问题，A_O_QUERY?
                    if nizhuan is True:
                        for A_O_index, A_O_item in enumerate(A_O_QUERY):
                            T_Q_A_O = self.tokenizer.convert_tokens_to_ids(
                                [word_.lower() for word in [A_O_item] for word_ in word])
                            l1 = len(T_Q_A_O)
                            T_Q_A_O = T_CLS + T_text + T_SEP + T_Q_A_O
                            _forward_A_O_query.append(T_Q_A_O)
                            _A_O_query_mask_temp = [1] * len(T_Q_A_O)  # 因为就1个，直接取0就行
                            _A_O_query_seg_temp = [0] * (len(T_text)+2) + [1] * (len(T_Q_A_O) - len(T_text)-2)
                            _forward_A_O_query_mask.append(_A_O_query_mask_temp)
                            _forward_A_O_query_seg.append(_A_O_query_seg_temp)

                            # 这里是不是会有问题？
                            _forward_A_O_answer_start_temp = [-1] + A_O_ANSWER[A_O_index][0]+[-1] * (l1 + 1)
                            _forward_A_O_answer_end_temp = [-1] + A_O_ANSWER[A_O_index][1]+[-1] * (l1 + 1)
                        ...
                        for A_O_index, A_O_item in enumerate(A_O_QUERY):
                            ...
                            # 这里是不是会有问题？
                            _forward_A_O_answer_start_temp = [-1] * (l1 + 2) + A_O_ANSWER[A_O_index][0]
                            _forward_A_O_answer_end_temp = [-1] * (l1 + 2) + A_O_ANSWER[A_O_index][1]
                    ...
                    ##################
                    if nizhuan is True:
                        for O_A_index, O_A_item in enumerate(O_A_QUERY):
                            T_Q_O_A = self.tokenizer.convert_tokens_to_ids(
                                [word_.lower() for word in [O_A_item] for word_ in word])
                            l1 = len(T_Q_O_A)
                            T_Q_O_A = T_CLS + T_text+ T_SEP + T_Q_O_A
                            _forward_O_A_query.append(T_Q_O_A)
                            _O_A_query_mask_temp = [1] * len(T_Q_O_A)  # 因为就1个，直接取0就行
                            _O_A_query_seg_temp = [0] * (len(T_text)+2) + [1] * (len(T_Q_O_A) - len(T_text)-2)
                            _forward_O_A_query_mask.append(_O_A_query_mask_temp)
                            _forward_O_A_query_seg.append(_O_A_query_seg_temp)

                            # 这里是不是会有问题？
                            _forward_O_A_answer_start_temp = [-1] + O_A_ANSWER[O_A_index][0]+[-1] * (l1 + 1)
                            _forward_O_A_answer_end_temp = [-1] + O_A_ANSWER[O_A_index][1]+[-1] * (l1 + 1)
                        ...
                        for O_A_index, O_A_item in enumerate(O_A_QUERY):
                            ...
                            _O_A_query_mask_temp = [1] * len(_forward_O_A_query[O_A_index])  # 因为就1个，直接取0就行
                            _O_A_query_seg_temp = [0] * (len(_forward_O_A_query[O_A_index]) - len(T_text)) + [1] * (
                                len(T_text))
                            _forward_O_A_query_mask.append(_O_A_query_mask_temp)
                            _forward_O_A_query_seg.append(_O_A_query_seg_temp)

                            _forward_O_A_answer_start_temp = [-1] * (len(T_Q_O_A) + 2) + O_A_ANSWER[O_A_index][0]
                            _forward_O_A_answer_end_temp = [-1] * (len(T_Q_O_A) + 2) + O_A_ANSWER[O_A_index][1]
                    ...
                    if nizhuan is True:
                        for AO_P_index, AO_P_item in enumerate(AO_P_QUERY):
                            ...
                            _forward_AO_P_query.append(T_CLS + T_text + T_SEP +  T_Q_AO_P)
                            _AO_P_query_mask_temp = [1] * len(_forward_AO_P_query[AO_P_index])  # 因为就1个，直接取0就行
                            _AO_P_query_seg_temp = [0] * ( len(T_text)+2 ) + [1] * (
                                        len(_forward_AO_P_query[AO_P_index]) -len(T_text)-2)
                    ...
                    #################
                    # for P->A  (和S_A)不一样，P_A可能可能是多轮的
                    #################
                    if nizhuan is True:
                        for P_A_index, P_A_item in enumerate(P_A_QUERY):
                            T_Q_P_A = self.tokenizer.convert_tokens_to_ids(
                                [word_.lower() for word in [P_A_item] for word_ in word])
                            l1 = len(T_Q_P_A)
                            T_Q_P_A = T_CLS + T_text + T_SEP + T_Q_P_A
                            _forward_P_A_query.append(T_Q_P_A)
                            _P_A_query_mask_temp = [1] * len(T_Q_P_A)  # 因为就1个，直接取0就行
                            _P_A_query_seg_temp = [0] * (len(T_text)+2) + [1] * (len(T_Q_P_A) - len(T_text)-2)
                            _forward_P_A_query_mask.append(_P_A_query_mask_temp)
                            _forward_P_A_query_seg.append(_P_A_query_seg_temp)

                            # 这里是不是会有问题？
                            _forward_P_A_answer_start_temp = [-1] + P_A_ANSWER[P_A_index][0]+[-1] * (l1 + 1)
                            _forward_P_A_answer_end_temp = [-1] + P_A_ANSWER[P_A_index][1]+[-1] * (l1 + 1)

                            _forward_P_A_answer_start.append(_forward_P_A_answer_start_temp)
                            _forward_P_A_answer_end.append(_forward_P_A_answer_end_temp)

                            assert len(T_Q_P_A) == len(_P_A_query_mask_temp)
                            assert len(T_Q_P_A) == len(_P_A_query_seg_temp)
                            assert len(T_Q_P_A) == len(_forward_P_A_answer_start_temp)
                            assert len(T_Q_P_A) == len(_forward_P_A_answer_end_temp)
                        # 自己的填充
                        P_A_max_len = max([len(l1) for l1 in _forward_P_A_query])
                        for P_A_index, P_A_item in enumerate(_forward_P_A_query):
                            _forward_P_A_query[P_A_index] += [0] * (P_A_max_len - len(P_A_item))
                        for P_A_index, P_A_item in enumerate(_forward_P_A_query_mask):
                            _forward_P_A_query_mask[P_A_index] += [0] * (P_A_max_len - len(P_A_item))
                        for P_A_index, P_A_item in enumerate(_forward_P_A_query_seg):
                            _forward_P_A_query_seg[P_A_index] += [1] * (P_A_max_len - len(P_A_item))
                        for P_A_index, P_A_item in enumerate(_forward_P_A_answer_start):
                            _forward_P_A_answer_start[P_A_index] += [-1] * (P_A_max_len - len(P_A_item))
                        for P_A_index, P_A_item in enumerate(_forward_P_A_answer_end):
                            _forward_P_A_answer_end[P_A_index] += [-1] * (P_A_max_len - len(P_A_item))

                        max_len_P_A_in = max(max_len_P_A_in, len(_forward_P_A_query[0]))
                        max_len_P_A_out = max(max_len_P_A_out, len(_forward_P_A_query))
                    else:
                        for P_A_index, P_A_item in enumerate(P_A_QUERY):
                            T_Q_P_A = self.tokenizer.convert_tokens_to_ids(
                                [word_.lower() for word in [P_A_item] for word_ in word])
                            l1 = len(T_Q_P_A)
                            T_Q_P_A = T_CLS + T_Q_P_A + T_SEP + T_text
                            _forward_P_A_query.append(T_Q_P_A)
                            _P_A_query_mask_temp = [1] * len(T_Q_P_A)  # 因为就1个，直接取0就行
                            _P_A_query_seg_temp = [0] * (len(T_Q_P_A) - len(T_text)) + [1] * len(T_text)
                            _forward_P_A_query_mask.append(_P_A_query_mask_temp)
                            _forward_P_A_query_seg.append(_P_A_query_seg_temp)

                            # 这里是不是会有问题？
                            _forward_P_A_answer_start_temp = [-1] * (l1 + 2) + P_A_ANSWER[P_A_index][0]
                            _forward_P_A_answer_end_temp = [-1] * (l1 + 2) + P_A_ANSWER[P_A_index][1]
                    ...
                    ##############
                    if nizhuan is True:
                        for P_O_index, P_O_item in enumerate(P_O_QUERY):
                            T_Q_P_O = self.tokenizer.convert_tokens_to_ids(
                                [word_.lower() for word in [P_O_item] for word_ in word])
                            l1 = len(T_Q_P_O)
                            T_Q_P_O = T_CLS + T_text + T_SEP + T_Q_P_O
                            _forward_P_O_query.append(T_Q_P_O)
                            _P_O_query_mask_temp = [1] * len(T_Q_P_O)  # 因为就1个，直接取0就行
                            _P_O_query_seg_temp = [0] * (len(T_text)+2) + [1] * (len(T_Q_P_O) - len(T_text)-2)
                            _forward_P_O_query_mask.append(_P_O_query_mask_temp)
                            _forward_P_O_query_seg.append(_P_O_query_seg_temp)

                            # 这里是不是会有问题？
                            _forward_P_O_answer_start_temp = [-1] + P_O_ANSWER[P_O_index][0]+[-1] * (l1 + 1)
                            _forward_P_O_answer_end_temp = [-1] + P_O_ANSWER[P_O_index][1]+[-1] * (l1 + 1)

                            _forward_P_O_answer_start.append(_forward_P_O_answer_start_temp)
                            _forward_P_O_answer_end.append(_forward_P_O_answer_end_temp)

                            assert len(T_Q_P_O) == len(_P_O_query_mask_temp)
                            assert len(T_Q_P_O) == len(_P_O_query_seg_temp)
                            assert len(T_Q_P_O) == len(_forward_P_O_answer_start_temp)
                            assert len(T_Q_P_O) == len(_forward_P_O_answer_end_temp)
                        # 自己的填充
                        P_O_max_len = max([len(l1) for l1 in _forward_P_O_query])
                        for P_O_index, P_O_item in enumerate(_forward_P_O_query):
                            _forward_P_O_query[P_O_index] += [0] * (P_O_max_len - len(P_O_item))
                        for P_O_index, P_O_item in enumerate(_forward_P_O_query_mask):
                            _forward_P_O_query_mask[P_O_index] += [0] * (P_O_max_len - len(P_O_item))
                        for P_O_index, P_O_item in enumerate(_forward_P_O_query_seg):
                            _forward_P_O_query_seg[P_O_index] += [1] * (P_O_max_len - len(P_O_item))
                        for P_O_index, P_O_item in enumerate(_forward_P_O_answer_start):
                            _forward_P_O_answer_start[P_O_index] += [-1] * (P_O_max_len - len(P_O_item))
                        for P_O_index, P_O_item in enumerate(_forward_P_O_answer_end):
                            _forward_P_O_answer_end[P_O_index] += [-1] * (P_O_max_len - len(P_O_item))

                        max_len_P_O_in = max(max_len_P_O_in, len(_forward_P_O_query[0]))
                        max_len_P_O_out = max(max_len_P_O_out, len(_forward_P_O_query))
                    else:
                        for P_O_index, P_O_item in enumerate(P_O_QUERY):
                            T_Q_P_O = self.tokenizer.convert_tokens_to_ids(
                                [word_.lower() for word in [P_O_item] for word_ in word])
                            l1 = len(T_Q_P_O)
                            T_Q_P_O = T_CLS + T_Q_P_O + T_SEP + T_text
                            _forward_P_O_query.append(T_Q_P_O)
                            _P_O_query_mask_temp = [1] * len(T_Q_P_O)  # 因为就1个，直接取0就行
                            _P_O_query_seg_temp = [0] * (len(T_Q_P_O) - len(T_text)) + [1] * len(T_text)
                            _forward_P_O_query_mask.append(_P_O_query_mask_temp)
                            _forward_P_O_query_seg.append(_P_O_query_seg_temp)

                            # 这里是不是会有问题？
                            _forward_P_O_answer_start_temp = [-1] * (l1 + 2) + P_O_ANSWER[P_O_index][0]
                            _forward_P_O_answer_end_temp = [-1] * (l1 + 2) + P_O_ANSWER[P_O_index][1]
                    ...
                    ##############
                    if nizhuan is True:
                        for PA_O_index, PA_O_item in enumerate(PA_O_QUERY):
                            T_Q_PA_O = self.tokenizer.convert_tokens_to_ids(
                                [word_.lower() for word in [PA_O_item] for word_ in word])
                            l1 = len(T_Q_PA_O)
                            T_Q_PA_O = T_CLS + T_text + T_SEP + T_Q_PA_O
                            _forward_PA_O_query.append(T_Q_PA_O)
                            _PA_O_query_mask_temp = [1] * len(T_Q_PA_O)  # 因为就1个，直接取0就行
                            _PA_O_query_seg_temp = [0] * (len(T_text)+2) + [1] * (len(T_Q_PA_O) - len(T_text)-2)
                            _forward_PA_O_query_mask.append(_PA_O_query_mask_temp)
                            _forward_PA_O_query_seg.append(_PA_O_query_seg_temp)

                            # 这里是不是会有问题？
                            _forward_PA_O_answer_start_temp = [-1] + PA_O_ANSWER[PA_O_index][0]+[-1] * (l1 + 1)
                            _forward_PA_O_answer_end_temp = [-1] + PA_O_ANSWER[PA_O_index][1]+[-1] * (l1 + 1)

                            _forward_PA_O_answer_start.append(_forward_PA_O_answer_start_temp)
                            _forward_PA_O_answer_end.append(_forward_PA_O_answer_end_temp)

                            assert len(T_Q_PA_O) == len(_PA_O_query_mask_temp)
                            assert len(T_Q_PA_O) == len(_PA_O_query_seg_temp)
                            assert len(T_Q_PA_O) == len(_forward_PA_O_answer_start_temp)
                            assert len(T_Q_PA_O) == len(_forward_PA_O_answer_end_temp)
                        # 自己的填充
                        PA_O_max_len = max([len(l1) for l1 in _forward_PA_O_query])
                        for PA_O_index, PA_O_item in enumerate(_forward_PA_O_query):
                            _forward_PA_O_query[PA_O_index] += [0] * (PA_O_max_len - len(PA_O_item))
                        for PA_O_index, PA_O_item in enumerate(_forward_PA_O_query_mask):
                            _forward_PA_O_query_mask[PA_O_index] += [0] * (PA_O_max_len - len(PA_O_item))
                        for PA_O_index, PA_O_item in enumerate(_forward_PA_O_query_seg):
                            _forward_PA_O_query_seg[PA_O_index] += [1] * (PA_O_max_len - len(PA_O_item))
                        for PA_O_index, PA_O_item in enumerate(_forward_PA_O_answer_start):
                            _forward_PA_O_answer_start[PA_O_index] += [-1] * (PA_O_max_len - len(PA_O_item))
                        for PA_O_index, PA_O_item in enumerate(_forward_PA_O_answer_end):
                            _forward_PA_O_answer_end[PA_O_index] += [-1] * (PA_O_max_len - len(PA_O_item))

                        max_len_PA_O_in = max(max_len_PA_O_in, len(_forward_PA_O_query[0]))
                        max_len_PA_O_out = max(max_len_PA_O_out, len(_forward_PA_O_query))
                    else:
                        for PA_O_index, PA_O_item in enumerate(PA_O_QUERY):
                            T_Q_PA_O = self.tokenizer.convert_tokens_to_ids(
                                [word_.lower() for word in [PA_O_item] for word_ in word])
                            l1 = len(T_Q_PA_O)
                            T_Q_PA_O = T_CLS + T_Q_PA_O + T_SEP + T_text
                            _forward_PA_O_query.append(T_Q_PA_O)
                            _PA_O_query_mask_temp = [1] * len(T_Q_PA_O)  # 因为就1个，直接取0就行
                            _PA_O_query_seg_temp = [0] * (len(T_Q_PA_O) - len(T_text)) + [1] * len(T_text)
                            _forward_PA_O_query_mask.append(_PA_O_query_mask_temp)
                            _forward_PA_O_query_seg.append(_PA_O_query_seg_temp)

                            # 这里是不是会有问题？
                            _forward_PA_O_answer_start_temp = [-1] * (l1 + 2) + PA_O_ANSWER[PA_O_index][0]
                            _forward_PA_O_answer_end_temp = [-1] * (l1 + 2) + PA_O_ANSWER[PA_O_index][1]
                    ...
                    #################
                    if nizhuan is True:
                        for PO_A_index, PO_A_item in enumerate(PO_A_QUERY):
                            T_Q_PO_A = self.tokenizer.convert_tokens_to_ids(
                                [word_.lower() for word in [PO_A_item] for word_ in word])
                            l1 = len(T_Q_PO_A)
                            T_Q_PO_A = T_CLS + T_text+ T_SEP + T_Q_PO_A
                            _forward_PO_A_query.append(T_Q_PO_A)
                            _PO_A_query_mask_temp = [1] * len(T_Q_PO_A)  # 因为就1个，直接取0就行
                            _PO_A_query_seg_temp = [0] * (len(T_text)+2) + [1] * (len(T_Q_PO_A) - len(T_text)-2)
                            _forward_PO_A_query_mask.append(_PO_A_query_mask_temp)
                            _forward_PO_A_query_seg.append(_PO_A_query_seg_temp)

                            # 这里是不是会有问题？
                            _forward_PO_A_answer_start_temp = [-1] + PO_A_ANSWER[PO_A_index][0]+[-1] * (l1 + 1)
                            _forward_PO_A_answer_end_temp = [-1] + PO_A_ANSWER[PO_A_index][1]+[-1] * (l1 + 1)

                            _forward_PO_A_answer_start.append(_forward_PO_A_answer_start_temp)
                            _forward_PO_A_answer_end.append(_forward_PO_A_answer_end_temp)

                            assert len(T_Q_PO_A) == len(_PO_A_query_mask_temp)
                            assert len(T_Q_PO_A) == len(_PO_A_query_seg_temp)
                            assert len(T_Q_PO_A) == len(_forward_PO_A_answer_start_temp)
                            assert len(T_Q_PO_A) == len(_forward_PO_A_answer_end_temp)
                        # 自己的填充
                        PO_A_max_len = max([len(l1) for l1 in _forward_PO_A_query])
                        for PO_A_index, PO_A_item in enumerate(_forward_PO_A_query):
                            _forward_PO_A_query[PO_A_index] += [0] * (PO_A_max_len - len(PO_A_item))
                        for PO_A_index, PO_A_item in enumerate(_forward_PO_A_query_mask):
                            _forward_PO_A_query_mask[PO_A_index] += [0] * (PO_A_max_len - len(PO_A_item))
                        for PO_A_index, PO_A_item in enumerate(_forward_PO_A_query_seg):
                            _forward_PO_A_query_seg[PO_A_index] += [1] * (PO_A_max_len - len(PO_A_item))
                        for PO_A_index, PO_A_item in enumerate(_forward_PO_A_answer_start):
                            _forward_PO_A_answer_start[PO_A_index] += [-1] * (PO_A_max_len - len(PO_A_item))
                        for PO_A_index, PO_A_item in enumerate(_forward_PO_A_answer_end):
                            _forward_PO_A_answer_end[PO_A_index] += [-1] * (PO_A_max_len - len(PO_A_item))

                        max_len_PO_A_in = max(max_len_PO_A_in, len(_forward_PO_A_query[0]))
                        max_len_PO_A_out = max(max_len_PO_A_out, len(_forward_PO_A_query))
                    else:
                        for PO_A_index, PO_A_item in enumerate(PO_A_QUERY):
                            T_Q_PO_A = self.tokenizer.convert_tokens_to_ids(
                                [word_.lower() for word in [PO_A_item] for word_ in word])

                            _forward_PO_A_query.append(T_CLS + T_Q_PO_A + T_SEP + T_text)
                            _PO_A_query_mask_temp = [1] * len(_forward_PO_A_query[PO_A_index])  # 因为就1个，直接取0就行
                            _PO_A_query_seg_temp = [0] * (len(_forward_PO_A_query[PO_A_index]) - len(T_text)) + [1] * (
                                len(T_text))
                            _forward_PO_A_query_mask.append(_PO_A_query_mask_temp)
                            _forward_PO_A_query_seg.append(_PO_A_query_seg_temp)

                            _forward_PO_A_answer_start_temp = [-1] * (len(T_Q_PO_A) + 2) + PO_A_ANSWER[PO_A_index][0]
                            _forward_PO_A_answer_end_temp = [-1] * (len(T_Q_PO_A) + 2) + PO_A_ANSWER[PO_A_index][1]

                            _forward_PO_A_answer_start.append(_forward_PO_A_answer_start_temp)
                            _forward_PO_A_answer_end.append(_forward_PO_A_answer_end_temp)
                            T_Q_PO_A = T_CLS + T_Q_PO_A + T_SEP + T_text
                            assert len(T_Q_PO_A) == len(_PO_A_query_mask_temp)
                            assert len(T_Q_PO_A) == len(_PO_A_query_seg_temp)
                            assert len(T_Q_PO_A) == len(_forward_PO_A_answer_start_temp)
                            assert len(T_Q_PO_A) == len(_forward_PO_A_answer_end_temp)
                ...
                result = {
                    ...
                    # (13)for gat
                    ######## 没padding
                    '_word_back':yingshe_origin,
                    'text_length':text_length,  # 文本的原始长度
                    'bert_length':bert_length,   # bert之后的长度（原文本）
                    'gat_bert_sequence':gat_bert_sequence,
                    'gat_aspect_indi':gat_aspect_indi,
                    'gat_bert_segments_ids':gat_bert_segments_ids,

                    # P->A
                    '_forward_P_A_query': _forward_P_A_query,
                    '_forward_P_A_query_mask': _forward_P_A_query_mask,
                    '_forward_P_A_query_seg': _forward_P_A_query_seg,
                    '_forward_P_A_answer_start': _forward_P_A_answer_start,
                    '_forward_P_A_answer_end': _forward_P_A_answer_end,

                    # P->O
                    '_forward_P_O_query': _forward_P_O_query,
                    '_forward_P_O_query_mask': _forward_P_O_query_mask,
                    '_forward_P_O_query_seg': _forward_P_O_query_seg,
                    '_forward_P_O_answer_start': _forward_P_O_answer_start,
                    '_forward_P_O_answer_end': _forward_P_O_answer_end,

                    # PA->O
                    '_forward_PA_O_query': _forward_PA_O_query,
                    '_forward_PA_O_query_mask': _forward_PA_O_query_mask,
                    '_forward_PA_O_query_seg': _forward_PA_O_query_seg,
                    '_forward_PA_O_answer_start': _forward_PA_O_answer_start,
                    '_forward_PA_O_answer_end': _forward_PA_O_answer_end,

                    # PO->A
                    '_forward_PO_A_query': _forward_PO_A_query,
                    '_forward_PO_A_query_mask': _forward_PO_A_query_mask,
                    '_forward_PO_A_query_seg': _forward_PO_A_query_seg,
                    '_forward_PO_A_answer_start': _forward_PO_A_answer_start,
                    '_forward_PO_A_answer_end': _forward_PO_A_answer_end,
            ...
            for item1 in self.QAs_r: # 将所有数据处理成max长度
                ...
                for A_O_index, A_O_item in enumerate(A_O_query):
                    A_O_query[A_O_index] += [0] * (max_len_A_O_in - len(A_O_item))
                for ind in range(len(A_O_query),max_len_A_O_out):
                    A_O_query.append([0]*max_len_A_O_in)

                for A_O_index, A_O_item in enumerate(A_O_query_mask):
                    A_O_query_mask[A_O_index] += [0] * (max_len_A_O_in - len(A_O_item))
                for ind in range(len(A_O_query_mask),max_len_A_O_out):
                    A_O_query_mask.append([0]*max_len_A_O_in)

                for A_O_index, A_O_item in enumerate(A_O_query_seg):
                    A_O_query_seg[A_O_index] += [1] * (max_len_A_O_in - len(A_O_item))
                for ind in range(len(A_O_query_seg),max_len_A_O_out):
                    A_O_query_seg.append([0]*max_len_A_O_in)

                for A_O_index, A_O_item in enumerate(A_O_answer_start):
                    A_O_answer_start[A_O_index] += [-1] * (max_len_A_O_in - len(A_O_item))
                for ind in range(len(A_O_answer_start),max_len_A_O_out):
                    A_O_answer_start.append([-1]*max_len_A_O_in)

                for A_O_index, A_O_item in enumerate(A_O_answer_end):
                    A_O_answer_end[A_O_index] += [-1] * (max_len_A_O_in - len(A_O_item))
                ...
                for O_A_index, O_A_item in enumerate(O_A_query):
                    O_A_query[O_A_index] += [0] * (max_len_O_A_in - len(O_A_item))
                for ind in range(len(O_A_query),max_len_O_A_out):
                    O_A_query.append([0]*max_len_O_A_in)

                for O_A_index, O_A_item in enumerate(O_A_query_mask):
                    O_A_query_mask[O_A_index] += [0] * (max_len_O_A_in - len(O_A_item))
                for ind in range(len(O_A_query_mask),max_len_O_A_out):
                    O_A_query_mask.append([0]*max_len_O_A_in)

                for O_A_index, O_A_item in enumerate(O_A_query_seg):
                    O_A_query_seg[O_A_index] += [1] * (max_len_O_A_in - len(O_A_item))
                for ind in range(len(O_A_query_seg),max_len_O_A_out):
                    O_A_query_seg.append([0]*max_len_O_A_in)

                for O_A_index, O_A_item in enumerate(O_A_answer_start):
                    O_A_answer_start[O_A_index] += [-1] * (max_len_O_A_in - len(O_A_item))
                for ind in range(len(O_A_answer_start),max_len_O_A_out):
                    O_A_answer_start.append([-1]*max_len_O_A_in)

                for O_A_index, O_A_item in enumerate(O_A_answer_end):
                    O_A_answer_end[O_A_index] += [-1] * (max_len_O_A_in - len(O_A_item))
                ...
                # for gat
                _word_back = item1['_word_back']

                # P_A
                P_A_query = item1['_forward_P_A_query']
                P_A_query_mask = item1['_forward_P_A_query_mask']
                P_A_query_seg = item1['_forward_P_A_query_seg']
                P_A_answer_start = item1['_forward_P_A_answer_start']
                P_A_answer_end = item1['_forward_P_A_answer_end']

                for P_A_index, P_A_item in enumerate(P_A_query):
                    P_A_query[P_A_index] += [0] * (max_len_P_A_in - len(P_A_item))
                for ind in range(len(P_A_query),max_len_P_A_out):
                    P_A_query.append([0]*max_len_P_A_in)

                for P_A_index, P_A_item in enumerate(P_A_query_mask):
                    P_A_query_mask[P_A_index] += [0] * (max_len_P_A_in - len(P_A_item))
                for ind in range(len(P_A_query_mask),max_len_P_A_out):
                    P_A_query_mask.append([0]*max_len_P_A_in)

                for P_A_index, P_A_item in enumerate(P_A_query_seg):
                    P_A_query_seg[P_A_index] += [1] * (max_len_P_A_in - len(P_A_item))
                for ind in range(len(P_A_query_seg),max_len_P_A_out):
                    P_A_query_seg.append([0]*max_len_P_A_in)

                for P_A_index, P_A_item in enumerate(P_A_answer_start):
                    P_A_answer_start[P_A_index] += [-1] * (max_len_P_A_in - len(P_A_item))
                for ind in range(len(P_A_answer_start),max_len_P_A_out):
                    P_A_answer_start.append([-1]*max_len_P_A_in)

                for P_A_index, P_A_item in enumerate(P_A_answer_end):
                    P_A_answer_end[P_A_index] += [-1] * (max_len_P_A_in - len(P_A_item))
                for ind in range(len(P_A_answer_end),max_len_P_A_out):
                    P_A_answer_end.append([-1]*max_len_P_A_in)

                # P_O
                P_O_query = item1['_forward_P_O_query']
                P_O_query_mask = item1['_forward_P_O_query_mask']
                P_O_query_seg = item1['_forward_P_O_query_seg']
                P_O_answer_start = item1['_forward_P_O_answer_start']
                P_O_answer_end = item1['_forward_P_O_answer_end']

                for P_O_index, P_O_item in enumerate(P_O_query):
                    P_O_query[P_O_index] += [0] * (max_len_P_O_in - len(P_O_item))
                for ind in range(len(P_O_query),max_len_P_O_out):
                    P_O_query.append([0]*max_len_P_O_in)

                for P_O_index, P_O_item in enumerate(P_O_query_mask):
                    P_O_query_mask[P_O_index] += [0] * (max_len_P_O_in - len(P_O_item))
                for ind in range(len(P_O_query_mask),max_len_P_O_out):
                    P_O_query_mask.append([0]*max_len_P_O_in)

                for P_O_index, P_O_item in enumerate(P_O_query_seg):
                    P_O_query_seg[P_O_index] += [1] * (max_len_P_O_in - len(P_O_item))
                for ind in range(len(P_O_query_seg),max_len_P_O_out):
                    P_O_query_seg.append([0]*max_len_P_O_in)

                for P_O_index, P_O_item in enumerate(P_O_answer_start):
                    P_O_answer_start[P_O_index] += [-1] * (max_len_P_O_in - len(P_O_item))
                for ind in range(len(P_O_answer_start),max_len_P_O_out):
                    P_O_answer_start.append([-1]*max_len_P_O_in)

                for P_O_index, P_O_item in enumerate(P_O_answer_end):
                    P_O_answer_end[P_O_index] += [-1] * (max_len_P_O_in - len(P_O_item))
                for ind in range(len(P_O_answer_end),max_len_P_O_out):
                    P_O_answer_end.append([-1]*max_len_P_O_in)
                # PA_O
                PA_O_query = item1['_forward_PA_O_query']
                PA_O_query_mask = item1['_forward_PA_O_query_mask']
                PA_O_query_seg = item1['_forward_PA_O_query_seg']
                PA_O_answer_start = item1['_forward_PA_O_answer_start']
                PA_O_answer_end = item1['_forward_PA_O_answer_end']

                for PA_O_index, PA_O_item in enumerate(PA_O_query):
                    PA_O_query[PA_O_index] += [0] * (max_len_PA_O_in - len(PA_O_item))
                for ind in range(len(PA_O_query),max_len_PA_O_out):
                    PA_O_query.append([0]*max_len_PA_O_in)

                for PA_O_index, PA_O_item in enumerate(PA_O_query_mask):
                    PA_O_query_mask[PA_O_index] += [0] * (max_len_PA_O_in - len(PA_O_item))
                for ind in range(len(PA_O_query_mask),max_len_PA_O_out):
                    PA_O_query_mask.append([0]*max_len_PA_O_in)

                for PA_O_index, PA_O_item in enumerate(PA_O_query_seg):
                    PA_O_query_seg[PA_O_index] += [1] * (max_len_PA_O_in - len(PA_O_item))
                for ind in range(len(PA_O_query_seg),max_len_PA_O_out):
                    PA_O_query_seg.append([0]*max_len_PA_O_in)

                for PA_O_index, PA_O_item in enumerate(PA_O_answer_start):
                    PA_O_answer_start[PA_O_index] += [-1] * (max_len_PA_O_in - len(PA_O_item))
                for ind in range(len(PA_O_answer_start),max_len_PA_O_out):
                    PA_O_answer_start.append([-1]*max_len_PA_O_in)

                for PA_O_index, PA_O_item in enumerate(PA_O_answer_end):
                    PA_O_answer_end[PA_O_index] += [-1] * (max_len_PA_O_in - len(PA_O_item))
                for ind in range(len(PA_O_answer_end),max_len_PA_O_out):
                    PA_O_answer_end.append([-1]*max_len_PA_O_in)
                # PO_A
                PO_A_query = item1['_forward_PO_A_query']
                PO_A_query_mask = item1['_forward_PO_A_query_mask']
                PO_A_query_seg = item1['_forward_PO_A_query_seg']
                PO_A_answer_start = item1['_forward_PO_A_answer_start']
                PO_A_answer_end = item1['_forward_PO_A_answer_end']

                for PO_A_index, PO_A_item in enumerate(PO_A_query):
                    PO_A_query[PO_A_index] += [0] * (max_len_PO_A_in - len(PO_A_item))
                for ind in range(len(PO_A_query),max_len_PO_A_out):
                    PO_A_query.append([0]*max_len_PO_A_in)

                for PO_A_index, PO_A_item in enumerate(PO_A_query_mask):
                    PO_A_query_mask[PO_A_index] += [0] * (max_len_PO_A_in - len(PO_A_item))
                for ind in range(len(PO_A_query_mask),max_len_PO_A_out):
                    PO_A_query_mask.append([0]*max_len_PO_A_in)

                for PO_A_index, PO_A_item in enumerate(PO_A_query_seg):
                    PO_A_query_seg[PO_A_index] += [1] * (max_len_PO_A_in - len(PO_A_item))
                for ind in range(len(PO_A_query_seg),max_len_PO_A_out):
                    PO_A_query_seg.append([0]*max_len_PO_A_in)

                for PO_A_index, PO_A_item in enumerate(PO_A_answer_start):
                    PO_A_answer_start[PO_A_index] += [-1] * (max_len_PO_A_in - len(PO_A_item))
                for ind in range(len(PO_A_answer_start),max_len_PO_A_out):
                    PO_A_answer_start.append([-1]*max_len_PO_A_in)

                for PO_A_index, PO_A_item in enumerate(PO_A_answer_end):
                    PO_A_answer_end[PO_A_index] += [-1] * (max_len_PO_A_in - len(PO_A_item))
                for ind in range(len(PO_A_answer_end),max_len_PO_A_out):
                    PO_A_answer_end.append([-1]*max_len_PO_A_in)
                ...
                result = {
                    ...
                    # #(13)gat
                    # '_word_back':np.array(_word_back),
                    # 'text_length': np.array(text_length),  # 文本的原始长度
                    # 'bert_length': np.array(bert_length),  # bert之后的长度（原文本）

                    # P->A
                    '_forward_P_A_query': np.array(P_A_query),
                    '_forward_P_A_query_mask': np.array(P_A_query_mask),
                    '_forward_P_A_query_seg': np.array(P_A_query_seg),
                    '_forward_P_A_answer_start': np.array(P_A_answer_start),
                    '_forward_P_A_answer_end': np.array(P_A_answer_end),

                    # P->O
                    '_forward_P_O_query': np.array(P_O_query),
                    '_forward_P_O_query_mask': np.array(P_O_query_mask),
                    '_forward_P_O_query_seg': np.array(P_O_query_seg),
                    '_forward_P_O_answer_start': np.array(P_O_answer_start),
                    '_forward_P_O_answer_end': np.array(P_O_answer_end),

                    # PA->O
                    '_forward_PA_O_query': np.array(PA_O_query),
                    '_forward_PA_O_query_mask': np.array(PA_O_query_mask),
                    '_forward_PA_O_query_seg': np.array(PA_O_query_seg),
                    '_forward_PA_O_answer_start': np.array(PA_O_answer_start),
                    '_forward_PA_O_answer_end': np.array(PA_O_answer_end),

                    # PO->A
                    '_forward_PO_A_query': np.array(PO_A_query),
                    '_forward_PO_A_query_mask': np.array(PO_A_query_mask),
                    '_forward_PO_A_query_seg': np.array(PO_A_query_seg),
                    '_forward_PO_A_answer_start': np.array(PO_A_answer_start),
                    '_forward_PO_A_answer_end': np.array(PO_A_answer_end),
...
if __name__ == '__main__':
    ...
    if temp_opt.model_name=='ROBMRC':
        test_data_path =  './data/14lap_test.pt'
        ...
        test_data = total_data['test']
        batch_generator_train_1 = Data1.generate_batches(dataset=wawa, shuffle=False, batch_size=2, gpu=False)
        test_dataset = Data2.ReviewDataset_BMRC(train_data, dev_data, test_data, 'test')
        batch_generator_train_2 = Data1.generate_batches(dataset=test_dataset, shuffle=False, batch_size=2, gpu=False)
