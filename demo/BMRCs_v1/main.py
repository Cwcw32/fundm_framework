...
from utils import get_logger,calculate_sentiment_loss,calculate_entity_loss,filter_unpaired
...
def test2(model, t, batch_generator, gold_list, beta, logger):
    model.eval()
    ...
    standard=torch.load('./data/14lap_standard.pt')['dev']

    for batch_index, batch_dict in enumerate(batch_generator):

        # a_o_p_target = gold_list[tri_index]['a_o_p_target']
        # # 提取出的aspect
        # a_target = gold_list[tri_index]['a_target']
        # # 提取出的opinion
        # o_target = gold_list[tri_index]['o_target']
        # # 对应的aspect opinion对
        # a_o_target = gold_list[tri_index]['a_o_target']
        # # 最后的（AO,P）,注意和a_o_p不一样哦
        # a_p_target = gold_list[tri_index]['a_p_target']

        triplets_target1 = gold_list[batch_index]['a_o_p_target']
        asp_target1 = gold_list[batch_index]['a_target']
        opi_target1 = gold_list[batch_index]['o_target']
        asp_opi_target1 = gold_list[batch_index]['a_o_target']
        asp_pol_target1 = gold_list[batch_index]['a_p_target']
        ...
        final_opi_ind_list = []
        # forward q_1
        batch_dict['forward_asp_answer_start']= batch_dict['_forward_S_A_answer_start'][0]
        batch_dict['forward_asp_query']= batch_dict['_forward_S_A_query'][0]
        batch_dict['forward_asp_query_mask']= batch_dict['_forward_S_A_query_mask'][0]
        batch_dict['forward_asp_query_seg']= batch_dict['_forward_S_A_query_seg'][0]
        ...
        batch_dict['backward_opi_answer_start']= batch_dict['_forward_S_A_answer_start'][0]
        batch_dict['backward_opi_query'] = batch_dict['_forward_S_A_query'][0]
        batch_dict['backward_opi_query_mask'] = batch_dict['_forward_S_A_query_mask'][0]
        batch_dict['backward_opi_query_seg'] = batch_dict['_forward_S_A_query_seg'][0]
    ...
    logger.info(
        'Aspect-Opinion - Precision: {}\tRecall: {}\tF1: {}'.format(precision_aspect_opinion, recall_aspect_opinion,
                                                                    f1_aspect_opinion))
    return f1
def test(model, tokenizer, batch_generator, gold_list, beta, logger,gpu, max_len,task_type):
    """
    # 暂时先用三元组作为例子
    :param model: 用的模型
    ...
    if task_type=='ASQP':
        for batch_index, batch_dict in enumerate(batch_generator):
            ...
            a_o_p_target = standard[batch_index]['as_op_po']
            # 提取出的aspect
            a_target = standard[batch_index]['aspects']
            # 提取出的opinion
            o_target = standard[batch_index]['opinions']
            # 对应的aspect opinion对
            a_o_target = standard[batch_index]['as_op']
            # 最后的（AO,P）,注意和a_o_p不一样哦
            a_p_target = standard[batch_index]['as_po']
        ...
        for batch_index, batch_dict in enumerate(batch_generator):
            ...
            a_o_target = gold_list[tri_index]['a_o_target']
            # 最后的（AO,P）,注意和a_o_p不一样哦
            a_p_target = gold_list[tri_index]['a_o_p_target']
...
def train(args):
    ...
    logger, fh, sh = get_logger(args.log_path)
    ...
    train_dataset = SYZDataset(opt=args, data_path=data_path, data_type=data_type, dataset_type='dev',task_type=task_type)
    ...
    for epoch in range(start_epoch, args.epoch_num + 1):
        ...
        model.train()
        model.zero_grad()
        batch_generator_train = Data.generate_batches(dataset=train_dataset, batch_size=args.batch_size,
                                                gpu=True)


        #test(model, tokenizer_1, batch_generator_2, train_dataset.standard, 0.10, logger,gpu='cuda', max_len=200,task_type=task_type) # for debug
        #continue
        # validation
        ...
        # logger.info("dev")  #
        #
        # dev_f1 = test(model, tokenizer_1, batch_generator_dev, dev_dataset.standard, 0.8, logger, gpu='cuda',
        ...
        for batch_index, batch_dict in enumerate(tqdm(batch_generator_train,total=len(train_dataset)/args.batch_size)):
            ...
            ###############
            f_aspect_start_scores, f_aspect_end_scores = model(batch_dict['_forward_S_A_query'].view(-1, batch_dict['_forward_S_A_query'].size(-1)),
                                                               batch_dict['_forward_S_A_query_mask'].view(-1, batch_dict['_forward_S_A_query'].size(-1)),
                                                               batch_dict['_forward_S_A_query_seg'].view(-1, batch_dict['_forward_S_A_query'].size(-1)), step=0)

            S_A_loss = calculate_entity_loss(f_aspect_start_scores, f_aspect_end_scores,
                                                     batch_dict['_forward_S_A_answer_start'].view(-1, batch_dict['_forward_S_A_query'].size(-1)),
                                                     batch_dict['_forward_S_A_answer_end'].view(-1, batch_dict['_forward_S_A_query'].size(-1)))

            # S_O
            f_aspect_start_scores, f_aspect_end_scores = model(batch_dict['_forward_S_O_query'].view(-1, batch_dict['_forward_S_O_query'].size(-1)),
                                                               batch_dict['_forward_S_O_query_mask'].view(-1, batch_dict['_forward_S_O_query'].size(-1)),
                                                               batch_dict['_forward_S_O_query_seg'].view(-1, batch_dict['_forward_S_O_query'].size(-1)), step=0)

            S_O_loss = calculate_entity_loss(f_aspect_start_scores, f_aspect_end_scores,
                                                     batch_dict['_forward_S_O_answer_start'].view(-1, batch_dict['_forward_S_O_query'].size(-1)),
                                                     batch_dict['_forward_S_O_answer_end'].view(-1, batch_dict['_forward_S_O_query'].size(-1)))

            # A_O
            f_aspect_start_scores, f_aspect_end_scores = model(batch_dict['_forward_A_O_query'].view(-1, batch_dict['_forward_A_O_query'].size(-1)),
                                                               batch_dict['_forward_A_O_query_mask'].view(-1, batch_dict['_forward_A_O_query'].size(-1)),
                                                               batch_dict['_forward_A_O_query_seg'].view(-1, batch_dict['_forward_A_O_query'].size(-1)), step=0)

            A_O_loss = calculate_entity_loss(f_aspect_start_scores, f_aspect_end_scores,
                                                     batch_dict['_forward_A_O_answer_start'].view(-1, batch_dict['_forward_A_O_query'].size(-1)),
                                                     batch_dict['_forward_A_O_answer_end'].view(-1, batch_dict['_forward_A_O_query'].size(-1)))

            # O_A

            f_aspect_start_scores, f_aspect_end_scores = model(batch_dict['_forward_O_A_query'].view(-1, batch_dict['_forward_O_A_query'].size(-1)),
                                                               batch_dict['_forward_O_A_query_mask'].view(-1, batch_dict['_forward_O_A_query'].size(-1)),
                                                               batch_dict['_forward_O_A_query_seg'].view(-1, batch_dict['_forward_O_A_query'].size(-1)), step=0)

            O_A_loss = calculate_entity_loss(f_aspect_start_scores, f_aspect_end_scores,
                                                     batch_dict['_forward_O_A_answer_start'].view(-1, batch_dict['_forward_O_A_query'].size(-1)),
                                                     batch_dict['_forward_O_A_answer_end'].view(-1, batch_dict['_forward_O_A_query'].size(-1)))

            # AO_P
            sentiment_scores = model(batch_dict['_forward_AO_P_query'].view(-1, batch_dict['_forward_AO_P_query'].size(-1)),
                                                               batch_dict['_forward_AO_P_query_mask'].view(-1, batch_dict['_forward_AO_P_query'].size(-1)),
                                                               batch_dict['_forward_AO_P_query_seg'].view(-1, batch_dict['_forward_AO_P_query'].size(-1)), step='P')
            AO_P_loss = calculate_sentiment_loss(
                sentiment_scores,
                batch_dict['_forward_AO_P_answer'].view(-1))
            # loss
            loss_sum = S_A_loss + S_O_loss + O_A_loss + A_O_loss + \
                       args.beta * AO_P_loss
            # 这里现在少了几个loss
            loss_sum.backward()
            optimizer.step()
        ...
        #dev_f1 = test(model, tokenizer_1, batch_generator_dev,dev_dataset.standard, 0.8, logger,gpu='cuda', max_len=200,task_type=task_type)
        dev_f1 = test2(model, tokenizer_1, batch_generator_dev,dev_dataset.standard, 0.8, logger)#,gpu='cuda', max_len=200,task_type=task_type)

        # test
        batch_generator_test = Data.generate_batches(dataset=test_dataset, batch_size=1, shuffle=False,
                                                     gpu=args.gpu)
        logger.info("test")
        #test_f1=test(model, tokenizer_1, batch_generator_test, test_dataset.standard, 0.8, logger,gpu='cuda', max_len=200,task_type=task_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch_num', type=int, default=5, help='训练的次数')
