...
if __name__ == '__main__':
    ...
    parse.add_argument('-grain', type=bool, default=False, help='True for aspect-level || while False for sentence-level')
    parse.add_argument('-grain_type', type=int, default=0, help='1 for Text-Aspect; 2 for Text-Aspect-Query; 3 for Aspect-Query; 4 for Text-Aspect-Image')
    parse.add_argument('-abl_path', type=str, default='')
    parse.add_argument('-region', type=bool, default=False)



    parse.add_argument('--local_rank', type=int, default=-1, help='ka')
    ...
    print('asepct?: ',opt.grain)
   # if opt.local_rank!=-1:
     #   opt.acc_batch_size=opt.acc_batch_size/torch.cuda.device_count()
    print(torch.cuda.is_available())
    print(str(opt.gpu_num))
    os.environ['CUDA_VISIBLE_DEVICES'] = str(opt.gpu_num)
    opt.epoch = opt.train_fuse_model_epoch + opt.epoch

    dt = datetime.now()
    opt.save_model_path = opt.save_model_path + '/' +'55-'+ dt.strftime('%Y-%m-%d-%H-%M-%S-%f') + '-'
    if opt.add_note != '':
    ...
    if opt.cuda is True:
        ...
        if len(opt.gpu_num) > 1:
            if opt.gpu0_bsz > 0:
                ...
                python -m torch.distributed.launch --nproc_per_node=2 mabsa.py
                """
                print('当前GPU编号：', opt.local_rank)

                torch.cuda.set_device(opt.local_rank) # 在进行其他操作之前必须先设置这个
                device=torch.device('cuda:'+f'{opt.local_rank}')
                cl_fuse_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(cl_fuse_model).to(device)
                torch.distributed.init_process_group(backend='nccl')
                cl_fuse_model = cl_fuse_model.cuda()
                cl_fuse_model = DistributedDataParallel(cl_fuse_model,device_ids=[opt.local_rank], find_unused_parameters=True)
        else:
            cl_fuse_model = cl_fuse_model.cuda()
    ...
    if opt.text_model == 'bert-base':
        tokenizer = BertTokenizer.from_pretrained(opt.abl_path+'bert-base-uncased/vocab.txt')

    if opt.data_type == 'HFM':
        data_path_root = opt.abl_path + 'dataset/data/HFM/'
        ...
        data_path_root = opt.abl_path + '../data/mm/TV/' + opt.data_type + '/' + opt.data_path_name + '/'
        train_data_path = data_path_root + 'train.json'
        dev_data_path = data_path_root + 'dev.json'
        test_data_path = data_path_root + 'test.json'

        photo_path = opt.abl_path + '../data/mm/TV/' + opt.data_type + '/data'
        image_coordinate = None
        data_translation_path = opt.abl_path + '../data/mm/TV/' + opt.data_type + '/' + opt.data_type + '_translation.json'
