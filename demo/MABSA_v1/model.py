...
class ModelParam:
    def __init__(self, texts=None, images=None, bert_attention_mask=None, text_image_mask=None, segment_token=None, image_coordinate_position_token=None,aspects=None,as_bert_attention_mask=None):
        self.texts = texts
        self.images = images
        ...
        self.as_bert_attention_mask = as_bert_attention_mask

    def set_data_param(self, texts=None, images=None, bert_attention_mask=None, text_image_mask=None, segment_token=None, image_coordinate_position_token=None,aspects=None,as_bert_attention_mask=None):
        self.texts = texts
        self.images = images
        self.bert_attention_mask = bert_attention_mask
        self.text_image_mask = text_image_mask
...
class ImageModel_region(nn.Module):
    def __init__(self, opt):
        super(ImageModel_region, self).__init__()
        if opt.image_model == 'resnet-152':
            self.resnet = cv_models.resnet152(pretrained=True)
        elif opt.image_model == 'resnet-101':
            self.resnet = cv_models.resnet101(pretrained=True)
    ...
    def forward(self, images):
        outputs=[]
        for item in images:
            image_encoder = self.resnet_encoder(item)
            # image_encoder = self.conv_output(image_encoder)
            image_cls = self.resnet_avgpool(image_encoder)
            image_cls = torch.flatten(image_cls, 1)
            outputs.append(image_cls)
        return outputs
...
class FuseModel(nn.Module):
    def __init__(self, opt):
        ...
        self.image_model_region = ImageModel_region(opt)
        ...
        self.image_config.is_decoder=True
        self.image_config.add_cross_attention=True
    ...
    def forward(self, text_inputs, bert_attention_mask, image_inputs, text_image_mask,aspect_inputs=None,as_bert_attention_mask=None):
        text_encoder = self.text_model(text_inputs, attention_mask=bert_attention_mask)
        if self.grain_type==2:
            as_text_encoder = self.text_model(aspect_inputs, attention_mask=as_bert_attention_mask)
            as_text_encoder = as_text_encoder.last_hidden_state
            as_text_init = self.text_change(as_text_encoder)
        text_cls = text_encoder.pooler_output
        text_encoder = text_encoder.last_hidden_state
        text_init = self.text_change(text_encoder)
        if self.region is True:
            image_clss=self.image_model_region(image_inputs)
            ############
            # 对每个图片都用CLS
            ############
            if self.image_output_type == 'all':
                ...
                image_init=self.image_cls_change(image_clss[0].unsqueeze(1))
                for item in image_clss[1:]:
                    image_cls_init = self.image_cls_change(item)
                    image_init = torch.cat((image_init, image_cls_init.unsqueeze(1)), dim=1)

            image_mask = text_image_mask[:, -image_init.size(1):]
            if self.grain_type == 2:
                as_extended_attention_mask = get_extended_attention_mask(as_bert_attention_mask, as_text_init.size())
            im_extended_attention_mask = get_extended_attention_mask(image_mask, image_init.size())

            if self.grain_type == 2:
                # a_i_mask=torch.matmul(as_bert_attention_mask,image_mask.T)
                image_init_a = self.image_encoder(as_text_init,
                                                  attention_mask=None,
                                                  head_mask=None,
                                                  encoder_hidden_states=image_init,
                                                  encoder_attention_mask=im_extended_attention_mask,
                ...
                # 这里不加mask的原因其实是im_mask为全1
                as_self_att = [x.mean(dim=1) for x in image_init_a.attentions]
                as_image_att = [x.mean(dim=1) for x in image_init_a.cross_attentions]
                # as_image_att=image_init_a.cross_attentions
                image_init = image_init_a.last_hidden_state
            else:
            ...
            # image_init_a = image_init.last_hidden_state

            text_image_cat = torch.cat((text_init, image_init), dim=1)
            if self.grain_type == 2:
                text_image_mask = text_image_mask[:, :text_image_cat.shape[1]]
            ...
            text_image_transformer = self.text_image_encoder(text_image_cat,
                                                             ...
                                                             return_dict=self.text_config.use_return_dict)
            text_image_self_att = [x.mean(dim=1) for x in text_image_transformer.attentions]
    ...
    def forward(self, data_orgin: ModelParam, data_augment: ModelParam = None, labels=None, target_labels=None):
        ...
        if self.grain_type==2:
            orgin_res, orgin_text_cls, orgin_image_cls = self.fuse_model(data_orgin.texts, data_orgin.bert_attention_mask,
                                                                     data_orgin.images, data_orgin.text_image_mask,data_orgin.aspects,data_orgin.as_bert_attention_mask)
        else:
            orgin_res, orgin_text_cls, orgin_image_cls = self.fuse_model(data_orgin.texts, data_orgin.bert_attention_mask,
        ...
        if data_augment :
            ...
            if self.grain_type == 2:
                augment_res, augment_text_cls, augment_image_cls = self.fuse_model(data_augment.texts, data_augment.bert_attention_mask,
                                                                               ...
                                                                               data_augment.aspects,data_augment.as_bert_attention_mask
                                                                               )
            else:
                augment_res, augment_text_cls, augment_image_cls = self.fuse_model(data_augment.texts, data_augment.bert_attention_mask,
                                                                               data_augment.images, data_augment.text_image_mask
