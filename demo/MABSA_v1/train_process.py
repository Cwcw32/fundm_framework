...
def train_process(opt, train_loader, dev_loader, test_loader, cl_model, critertion, log_summary_writer:SummaryWriter=None, tokenizer=None, image_id_list=None,lrank=-1):
    optimizer = None
    ...
    for epoch in trange(opt.epoch, desc='Epoch:'):
        ...
        for index, data in enumerate(train_loader_tqdm):
            ...
            elif opt.grain_type==2:
                texts_origin, bert_attention_mask, image_origin, text_image_mask, labels, \
                    texts_augment, bert_attention_mask_augment, image_augment, text_image_mask_augment, target_labels,aspects_origin,as_bert_attention_mask = data
            if opt.cuda is True:
                if opt.local_rank==-1:
                    texts_origin = texts_origin.cuda()
                    bert_attention_mask = bert_attention_mask.cuda()
                    if opt.region is False:
                        image_origin = image_origin.cuda()
                        image_augment = image_augment.cuda()
                    else:
                        image_origin = [item.cuda() for item in image_origin]
                        image_augment = [item.cuda() for item in image_augment]
                    ...
                    if opt.grain_type == 2:
                        aspects_origin=aspects_origin.cuda()
                        as_bert_attention_mask=as_bert_attention_mask.cuda()
            ...
            if opt.grain_type==2:
                orgin_param.set_data_param(texts=texts_origin, bert_attention_mask=bert_attention_mask,
                                           images=image_origin, text_image_mask=text_image_mask,aspects=aspects_origin,as_bert_attention_mask=as_bert_attention_mask)
                # 翻译策略还没写，所以先弄这个
                augment_param.set_data_param(texts=texts_augment, bert_attention_mask=bert_attention_mask_augment,
                                           images=image_augment, text_image_mask=text_image_mask_augment,aspects=aspects_origin,as_bert_attention_mask=as_bert_attention_mask)

            origin_res, l_pos_neg, cl_lables, cl_self_loss = cl_model(orgin_param, augment_param, labels, target_labels)
            #origin_res=cl_model(orgin_param, augment_param, labels, target_labels)
            ...
            loss.backward()
            train_loader_tqdm.set_description("Train Iteration, loss: %.6f, lr: %e" %(loss, optimizer.param_groups[0]['lr']))
