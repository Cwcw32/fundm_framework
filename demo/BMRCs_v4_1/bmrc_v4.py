...
def test_bmrc(model, t, batch_generator, standard, beta, logger,opt=None):
    model.eval()
    ...
    for batch_index, batch_dict in enumerate(batch_generator):
        ...
        for idx in range(len(triplets_predict)):
            ...
            PO_A_query = t.convert_tokens_to_ids(
                [word.lower() if word not in ['[CLS]', '[SEP]'] else word for word in text1.split()])
            opi=[]
            if triplets_predict[idx][2]==triplets_predict[idx][3]:
                opi=[passenge[triplets_predict[idx][2]].item()]
                PO_A_query+=[passenge[triplets_predict[idx][2]].item()]
            else:
                for j in range(triplets_predict[idx][2], triplets_predict[idx][3]+1):  # 因为是span所以要循环
                    PO_A_query.append(passenge[j].item())
                    opi.append(passenge[j].item())
            PO_A_query.append(t.convert_tokens_to_ids('?'))
            PO_A_query.append(t.convert_tokens_to_ids('[SEP]'))
            PO_A_query_seg = [0] * len(PO_A_query)
            PO_A_length = len(PO_A_query) # 这个长度是要算的

            PO_A_query = torch.tensor(PO_A_query).long().cuda()
            PO_A_query = torch.cat([PO_A_query, passenge], -1).unsqueeze(0)
            PO_A_query_seg += [1] * passenge.size(0)
            PO_A_query_mask = torch.ones(PO_A_query.size(1)).float().cuda().unsqueeze(0)
            PO_A_query_seg = torch.tensor(PO_A_query_seg).long().cuda().unsqueeze(0)

            PO_A_start_scores, PO_A_end_scores = model(PO_A_query, PO_A_query_mask, PO_A_query_seg, 0)

            PO_A_start_scores = F.softmax(PO_A_start_scores[0], dim=1)
            PO_A_end_scores = F.softmax(PO_A_end_scores[0], dim=1)
            PO_A_start_prob, PO_A_start_ind = torch.max(PO_A_start_scores, dim=1)
            PO_A_end_prob, PO_A_end_ind = torch.max(PO_A_end_scores, dim=1)

            PO_A_start_prob_temp = []
            PO_A_end_prob_temp = []
            PO_A_start_index_temp = []
            PO_A_end_index_temp = []

            for k in range(PO_A_start_ind.size(0)):
                if PO_A_query_seg[0, k] == 1:
                    if PO_A_start_ind[k].item() == 1:
                        PO_A_start_index_temp.append(k)
                        PO_A_start_prob_temp.append(PO_A_start_prob[k].item())
                    if PO_A_end_ind[k].item() == 1:
                        PO_A_end_index_temp.append(k)
                        PO_A_end_prob_temp.append(PO_A_end_prob[k].item())
            #print(1)
            # # 第二次过滤？
            PO_A_start_index, PO_A_end_index, PO_A_prob = utils.filter_unpaired_robmrc(
                PO_A_start_prob_temp, PO_A_end_prob_temp, PO_A_start_index_temp, PO_A_end_index_temp, max_len=200)

            # # pair的过滤
            for idy in range(len(PO_A_start_index)):
                asp = [PO_A_query[0][j].item() for j in range(PO_A_start_index[idy], PO_A_end_index[idy] + 1)]
                opi_ind = [triplets_predict[idx][2], triplets_predict[idx][3]]  # 因为提问的长度是5，也就是说 [CLS] What aspects ? [SEP] 对应的长度是5，故减掉就是在原句中的？（说对应是因为一个单词长度不一定是1）
                asp_ind = [PO_A_start_index[idy] - PO_A_length,PO_A_end_index[idy] - PO_A_length]  # 因为aspect的长度不同，所以这里是之前算的问题Q的长度，减去之后就是原位置
                temp_prob = PO_A_prob[idy]
                if opt is not None:
    ...
    for epoch in range(start_epoch, args.epoch_num + 1):
        ...
        if True:
            ...
            if dev_f1 > best_f1:
                ...
                torch.save(state, args.save_model_path+'.pth')
...
if __name__ == '__main__':
    ...
    opt.train_rt=True
    opt.mode="train"
    #opt.checkpoint_path= 'checkpoint/2.pth'
