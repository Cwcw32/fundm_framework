...
def train(args):
    ...
    train_dataset = SYZDataset(opt=args, data_path=data_path, data_type=data_type, dataset_type='train',
                               task_type=task_type)
    dev_dataset = SYZDataset(opt=args, data_path=data_path, data_type=data_type, dataset_type='dev',
                             task_type=task_type)
    ...
    for epoch in range(start_epoch, args.epoch_num + 1):
        ...
        for batch_index, batch_dict in enumerate(
                tqdm(batch_generator_train, total=len(train_dataset) / args.batch_size)):
            optimizer.zero_grad()
            ################
            ###  不要有的加view有的不加，提前先写好数据处理（待写
            ###############
            f_aspect_start_scores, f_aspect_end_scores = model(batch_dict['_forward_S_A_query'],
                                                               batch_dict['_forward_S_A_query_mask'],
                                                               batch_dict['_forward_S_A_query_seg'], step=0)

            S_A_loss = utils.calculate_entity_loss(f_aspect_start_scores, f_aspect_end_scores,
                                                   batch_dict['_forward_S_A_answer_start'].view(-1, batch_dict[
                                                       '_forward_S_A_query'].size(-1)),
                                                   batch_dict['_forward_S_A_answer_end'].view(-1, batch_dict[
                                                       '_forward_S_A_query'].size(-1)))

            # S_O
            f_aspect_start_scores, f_aspect_end_scores = model(batch_dict['_forward_S_O_query'],
                                                               batch_dict['_forward_S_O_query_mask'],
                                                               batch_dict['_forward_S_O_query_seg'], step=0)

            S_O_loss = utils.calculate_entity_loss(f_aspect_start_scores, f_aspect_end_scores,
                                                   batch_dict['_forward_S_O_answer_start'].view(-1, batch_dict[
                                                       '_forward_S_O_answer_start'].size(-1)),
                                                   batch_dict['_forward_S_O_answer_end'].view(-1, batch_dict[
                                                       '_forward_S_O_answer_end'].size(-1)))

            # A_O
            f_aspect_start_scores, f_aspect_end_scores = model(
                batch_dict['_forward_A_O_query'].view(-1, batch_dict['_forward_A_O_query'].size(-1)),
            ...
            A_O_loss = utils.calculate_entity_loss(f_aspect_start_scores, f_aspect_end_scores,
                                                   batch_dict['_forward_A_O_answer_start'].view(-1, batch_dict[
                                                       '_forward_A_O_answer_start'].size(-1)),
                                                   batch_dict['_forward_A_O_answer_end'].view(-1, batch_dict[
                                                       '_forward_A_O_answer_end'].size(-1)))

            # O_A

            f_aspect_start_scores, f_aspect_end_scores = model(
                batch_dict['_forward_O_A_query'].view(-1, batch_dict['_forward_O_A_query'].size(-1)),
            ...
            O_A_loss = utils.calculate_entity_loss(f_aspect_start_scores, f_aspect_end_scores,
                                                   batch_dict['_forward_O_A_answer_start'].view(-1, batch_dict[
                                                       '_forward_O_A_answer_start'].size(-1)),
                                                   batch_dict['_forward_O_A_answer_end'].view(-1, batch_dict[
                                                       '_forward_O_A_answer_end'].size(-1)))

            # AO_P
            sentiment_scores = model(
                batch_dict['_forward_AO_P_query'].view(-1, batch_dict['_forward_AO_P_query'].size(-1)),
                batch_dict['_forward_AO_P_query_mask'].view(-1, batch_dict['_forward_AO_P_query_mask'].size(-1)),
                batch_dict['_forward_AO_P_query_seg'].view(-1, batch_dict['_forward_AO_P_query_seg'].size(-1)),
                step='P')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_type', type=str, default='ASTE', choices=['lap14', 'rest14', 'rest15', 'rest16'],
                        help='要对哪个任务进行实验')
