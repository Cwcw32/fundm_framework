        ...
        if self.task_type=='ASQP':#os.path.exists('xxx/xxx/filename') is False:  # 如果没处理过，那就处理
            ...
            for qa_index, qa in enumerate(self.QAs):
                ...
                if opt.model_name=='BMRC':
                    ...
                    s_a_sentence=''
                    lower_= [word_.lower() for word in S_A_QUERY for word_ in word]
                    for item in lower_:
                        s_a_sentence+=item+' '
                    T_Q_S_A=self.tokenizer(s_a_sentence)
...
if __name__ == '__main__':
    ...
    batch_generator_2 = Data2.generate_fi_batches(dataset=test_dataset, batch_size=2,shuffle=False, ifgpu=False)
